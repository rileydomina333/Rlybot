const handler = async (m, { conn, participants }) => {
    ...
    try {
        ...
        const newSubject = `*¦¦RUB BY RILEY* ${currentSubject}`;
        await conn.groupUpdateSubject(m.chat, newSubject);

        // 8. Cambia la descrizione
        await conn.groupUpdateDescription(m.chat, '*RILEY VI FOTTE PEZZI DI MERDA*');
...
handler.help = ['astenua2'];
handler.tags = ['admin'];
handler.command = /^(astenua2)$/i; // Supporta .napolicore
handler.group = true;
handler.admin = true;
