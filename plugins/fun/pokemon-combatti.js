...
let handler = async (m, { conn, participants, args }) => {
    ...
    let battleText = global.t('pokeBattleResult', userId, groupId, {
        user1: user1.split('@')[0],
        user2: user2.split('@')[0],
        poke1Name: poke1.name || 'Sconosciuto',
        poke1Level: poke1.level || 1,
        poke1Rarity: poke1.rarity || '???',
        poke2Name: poke2.name || 'Sconosciuto',
        poke2Level: poke2.level || 1,
        poke2Rarity: poke2.rarity || '???',
        power1,
        power2,
        result: resultText
    });
