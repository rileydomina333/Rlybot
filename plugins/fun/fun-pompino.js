...
let handler = async (m, { conn }) => {
  ...
  const sent = await conn.sendMessage(
    chatId,
    {
      text: `*Ora faccio un pompino a ${tag(destinatario)}...* 😏`,
  ...
  await editMessage(
    ...
    chatId,
    key,
`*🤤 Ohhsyy babyy* 🥵\n\n ${tag(mittente)} *ha fatto un pompino a ${tag(destinatario)} e gli ha sborrato in bocca in ${elapsed} secondi! 💦*`,
    [mittente, destinatario]
  )
}

handler.help = ['pompino @utente']
handler.tags = ['fun']
handler.command = ['pompino', 'succhia', 'bocchino']
handler.group = true
