...
let handler = async (m, { conn, args, command }) => {
    ...
    if (command === 'accetta' || command === 'accept') {
        ...
        let fromPokemons = users[from]?.pokemons || [];
        let acceptorPokemons = users[sender]?.pokemons || [];

        let fromPoke = fromPokemons[myIndex];
        let acceptorPoke = acceptorPokemons[theirIndex];

        if (!fromPoke || !acceptorPoke) {
            delete tradeRequests[sender];
            return m.reply(global.t('tradeUnavailable', userId, groupId));
        }

        users[from].pokemons[myIndex] = acceptorPoke;
        users[sender].pokemons[theirIndex] = fromPoke;
        ...
        return m.reply(global.t('tradeSuccess', userId, groupId, {
            from: from.split('@')[0],
            to: sender.split('@')[0],
            poke1: fromPoke.name || fromPoke,
            poke2: acceptorPoke.name || acceptorPoke
